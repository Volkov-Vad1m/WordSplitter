 ...
 *
 * Функция берет слово, выделяет место для слова с переносами, затем расставляет
 *
 * \param word - слово в котором расставляем переносы
 *
 * правила расстановки слов - \ref rules
 */
...
int* super_sounds(wchar_t* sampled_word, int* n)
{
	...
	{
		if ((sampled_word[i - 1] == L'г') && (sampled_word[i] == L'с') && (sampled_word[i + 1] == L'с') && (sampled_word[i + 2] == L'с'))
		{
			places[k] = i;
			k++;
		}
...
int* ultraclose_sounds(wchar_t* sampled_word, int* n)
{
    ...
    for (int i = 0; i < wcslen(sampled_word); i++)
        places[i] = 0;
